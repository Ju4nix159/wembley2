...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['añadir_producto'])) {
  ...
  if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['añadir_categoria'])) {
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST'&& isset($_POST['editar_informacion_personal'])  ) {
        ...
        $updateUsuario = $con->prepare("UPDATE info_usuario SET nombre = :nombre, apellido = :apellido, dni = :dni, edad = :edad, sexo = :sexo, numtelefonico = :numtelefonico WHERE id_usuario = :id_usuario");
        ...
        $updateUsuario->bindParam(':edad', $edad, PDO::PARAM_INT);
        $updateUsuario->bindParam(':sexo', $sexo, PDO::PARAM_STR);
        $updateUsuario->bindParam(':numtelefonico', $numtelefonico, PDO::PARAM_STR);
