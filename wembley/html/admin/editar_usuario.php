...
<?php
  ...
  $sql_email_usuario = $con->prepare("SELECT * FROM usuarios WHERE id_usuario = :id_usuario");
  ...
  if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['editar_usuario'])) {
    ...
    $sql_actualizar_usuario = $con->prepare("UPDATE usuarios SET id_permiso = :id_permiso , id_estado_usuario = :id_estado_usuario WHERE id_usuario = :id_usuario");
    ...
    $sql_actualizar_usuario->bindParam(':id_estado_usuario', $id_estado_usuario, PDO::PARAM_INT);
                ...
                <div class="card-body">
                  ...
                  <div class="form-group row ">
                    ...
                    <div class="col-sm-10">
                      ...
                      <?php 
                        $sql_nombre_permisos = $con->prepare("SELECT id_permiso, nombre_permiso FROM permisos");
                      ...
                      <select class="form-control" name="id_permiso">
                        <?php foreach ($nombres_permisos as $row) {
                          ...
                          echo '<option value="' . $row["id_permiso"] . '" ' . $selected . '>' . $row["nombre_permiso"] . '</option>';
                        } ?>
                      </select>
                    </div>
                  </div>
                  ...
                  <div class="form-group row ">
                    ...
                    <div class="col-sm-10">
                      ...
                      <?php 
                        $sql_nombre_estado = $con->prepare("SELECT * FROM estado_usuario");
                        $sql_nombre_estado -> execute();
                        $nombres_estado = $sql_nombre_estado->fetchAll();
                      ?>
                      <select class="form-control" name="id_estado_usuario">
                        <?php foreach ($nombres_estado as $row) {
                          $selected = ($row["id_estado_usuario"] == $email_usuario["id_estado_usuario"]) ? "selected" : "";
                          echo '<option value="' . $row["id_estado_usuario"] . '" ' . $selected . '>' . $row["nombre_estado_usuario"] . '</option>';
                        } ?>
                      </select>
                    </div>
                  </div>
