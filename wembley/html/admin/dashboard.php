...
<?php
  ...
  $conexion = new mysqli("localhost", "root", "", "sarmienspace_db_tienda_2");
  ...
  $sql_cantidad_usuarios_genero = "SELECT s.nombre AS nombre_genero, COUNT(*) as cantidad 
  FROM info_usuario AS iu
  INNER JOIN sexo AS s ON iu.id_sexo = s.id_sexo
  GROUP BY iu.id_sexo";
  $resultado_cantidad_usuarios_genero = $conexion->query($sql_cantidad_usuarios_genero);
  
  // Arreglo para almacenar los datos del gráfico
  $datos_grafico = array();
  $labels = array();
  
  // Iterar sobre los resultado_cantidad_usuarioss y almacenarlos en el arreglo
  while ($fila = $resultado_cantidad_usuarios_genero->fetch_assoc()) {
  ...
  $resultado_ventas_por_mes = $conexion->query($sql_ventas_por_mes);

  $data_line = [];

  while ($fila = $resultado_ventas_por_mes->fetch_assoc()) {
  ...
  $resultado_cantidad_usuarios = $conexion->query($sql_cantidad_usuarios);
  if ($resultado_cantidad_usuarios) {
    // Obtener el resultado como un array asociativo
    $fila = $resultado_cantidad_usuarios->fetch_assoc();
  ...
  $resultado_cantidad_ventas = $conexion->query($sql_ventas_totales);
  if ($resultado_cantidad_ventas) {
    // Obtener el resultado como un array asociativo
    $fila = $resultado_cantidad_ventas->fetch_assoc();
...
/* GANANCIA TOTAL */
  ...
  $resultado_ganancia_total = $conexion->query($sql_ganancia_total);
  if ($resultado_ganancia_total) {
    // Obtener el resultado como un array asociativo
    $fila = $resultado_ganancia_total->fetch_assoc();
